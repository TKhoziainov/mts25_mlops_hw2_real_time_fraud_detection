  ...
  create or replace view `fraud_detection`.`model_run_results` 
...
with dbt_run_results as (
    select * from `fraud_detection`.`dbt_run_results`
),

dbt_models as (
    select * from `fraud_detection`.`dbt_models`
)
...
SELECT
    ...
    date_trunc('day', cast(run_results.generated_at as DateTime))
